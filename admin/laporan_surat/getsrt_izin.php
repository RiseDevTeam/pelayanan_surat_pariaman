...
if($_GET['kategori']=='bangunan'){
    $ktgr = "Surat Izin Bangunan";
    $loc = "surat_izin_bangunan";
} else if($_GET['kategori']=='usaha'){
    $ktgr = "Surat Izin Usaha";
...
<body>
    <table id="zero_config" class="table table-bordered no-wrap table-hover ">
        ...
        <?php
        $sql = mysqli_query($conn,"SELECT * FROM laporan_surat_izin a
        INNER JOIN pengajuan_surat_izin b
        ...
        ?>
            <tr>
                ...
                <td>
                <embed src="../../../files/pengajuan_surat/<?php echo $loc ?>/<?php echo $folder ?>/<?php echo $data['file_surat_izin'] ?>"
                style="width: 200px;">
                </td>
            </tr>
        <?php } } else { ?>
