...
$usarr = mysqli_query($conn,"SELECT * FROM user WHERE nomor_ktp = '$_SESSION[nomor_ktp]'");
$usar = mysqli_fetch_assoc($usarr);
} else {
        ...
        <nav id="navbar" class="navbar">
            <ul>
                ...
                <?php
                    if(isset($_SESSION['nomor_ktp'])){
                        echo"
                        <li><a class='nav-link scrollto active' href='surat_pengajuan.php'>Surat Pengajuan Saya</a></li>
    ...
    <section id="portfolio" class="portfolio">
      <div class="container" data-aos="fade-up">
        ...
        <div class="section-title">
          <h2>Surat Pengajuan Saya</h2>
        </div>

        <ul id="portfolio-flters" class="d-flex justify-content-center" data-aos="fade-up" data-aos-delay="100">
          <li data-filter="*" class="filter-active">Semua</li>
          <li data-filter=".filter-izin">Surat Izin</li>
          <li data-filter=".filter-rek">Surat Rekomendasi</li>
        </ul>
        ...
        <div class="row portfolio-container" data-aos="fade-up" data-aos-delay="200">
          ...
          WHERE id_user = '$usar[id_user]'
          ORDER BY id_pengajuan_surat_izin DESC");
          if(mysqli_num_rows($sql)>0){
              while($data = mysqli_fetch_array($sql)){
                $lapor = mysqli_query($conn,"SELECT * FROM laporan_surat_izin
                WHERE id_pengajuan_surat_izin = '$data[id_pengajuan_surat_izin]'");
                $dataa = mysqli_fetch_assoc($lapor);
                ?>

            <div class="card p-0 portfolio-item filter-izin">
                ...
                ?>" style="color: white"><?php echo $data['status']  ?></h5>
                <div class="card-body">
                    <h5 class="card-title"><?php echo $data['kategori_surat'] ?></h5>
                    <p class="card-text">Tanggal Surat Diajukan: <?php echo $data['tanggal'] ?>
                    <?php
                    if($data['status']!="Pending"){
                        echo"<br>Tanggal Surat ".$data['status'].": ".$dataa['tanggall'];
                        echo"</p>
                        <p>Keterangan:<br>".$dataa['keterangan'];
                    }
                    ?>
                    </p>
                    <?php
                    if(mysqli_num_rows($lapor)==1){?>
                      <a href="download_surat_izin.php?kategori_surat=<?php echo $data['kategori_surat']?>&id_surat=<?php echo $data['id_pengajuan_surat_izin'] ?>" 
                      class="btn btn-outline-primary">Download File Surat Izin</a>
                      <span><?php echo $dataa['file_surat_izin'] ?></span>
                  <?php } else {  }
                    ?>
          ...
          <?php
          $ssql = mysqli_query($conn,"SELECT * FROM pengajuan_surat_rekomendasi
          WHERE id_user = '$usar[id_user]'
          ORDER BY id_pengajuan_surat_rekomendasi DESC");
          if(mysqli_num_rows($ssql)>0){
              while($sdata = mysqli_fetch_array($ssql)){
                  $sel = mysqli_query($conn,"SELECT * FROM laporan_surat_rekomendasi
                  WHERE id_pengajuan_surat_rekomendasi = '$sdata[id_pengajuan_surat_rekomendasi]'");
                  if(mysqli_num_rows($sel)==1){
                    $data_rek = mysqli_fetch_assoc($sel);
                    $bg = "bg-success";
                    $txt = "Dikonfirmasi";
                  } else {
                    $bg = "bg-primary";
            ...
            <div class="card p-0 portfolio-item filter-rek">
                <h5 class="card-header <?php echo $bg; ?>" style="color: white"><?php echo $txt; ?></h5>
                <div class="card-body">
                    <h5 class="card-title"><?php echo $sdata['kategori_surat'] ?></h5>
                    <p class="card-text">Tanggal Surat Diajukan: <?php echo $sdata['tanggal'] ?>
                    <?php
                    if(mysqli_num_rows($sel)==1){ ?>
                    <br>Tanggal Surat Dikonfirmasi: "<?php echo $data_rek['tanggall']; ?>
                    </p>
                    <p>
                      Keterangan:<br>
                      <?php echo $data_rek['keterangan']; ?>
                    </p>
                    <a href="download_surat_rekomendasi.php?kategori_surat=<?php echo $sdata['kategori_surat']?>&id_surat=<?php echo $sdata['id_pengajuan_surat_rekomendasi'] ?>" 
                    class="btn btn-outline-primary">Download File Surat Izin</a>
                    <span><?php echo $data_rek['file_surat_rekomendasi'] ?></span>
                    <?php } else {}
                    ?>
